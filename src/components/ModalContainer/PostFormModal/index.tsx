  ...
  return (
    <div className="modal-overlay" onMouseDown={closeModalWindow}>
      <div className="modal-wrapper post-form-modal">
        ...
        <div className={`b-button p-font clickable post-form-modal__button ${state.formValid ? '' : 'disabled'}`} onClick={state.formValid ? handlePostForm : undefined}>{data.id ? 'Edit post' : 'Add post'}</div>
      </div>
    </div>
  )
};
